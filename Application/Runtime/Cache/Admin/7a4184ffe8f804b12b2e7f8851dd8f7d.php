    ...
    <?php if(($_SESSION['type']) == "tomin"): ?><li>
            ...
            <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
                <p class="pull-right" style="margin-top: 20px;">
                    <a class="btn btn-success" href="/aiyingshi/Admin/Article/add?id=<?php echo ($item["id"]); ?>">添加</a>
                </p>
                <h2 class="sub-header">影评管理</h2>
                        ...
                        <tbody>
                            <form id="list">
                                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$item): $mod = ($k % 2 );++$k;?><tr>
                                        <td><?php echo ($k); ?></td>
                                        <td>
                                            <img class="img-rounded img-responsive" src="/aiyingshi/Public/<?php echo ($item["poster"]); ?>" />
                                        </td>
                                        <td><?php echo ($item["title"]); ?></td>
                                        <td><?php echo ($item["addtime"]); ?></td>
                                        <td>
                                            <a class="btn btn-success" href="/aiyingshi/Admin/Article/change?id=<?php echo ($item["id"]); ?>">修改</a>
                                            <a class="btn btn-danger" href="/aiyingshi/Admin/Article/delete?id=<?php echo ($item["id"]); ?>">删除</a>
                                        </td>
